...
int Tree::StatTree(Link * from)	{
	int c =0;
	if (!from->isRoot()){
		string name = from->GetBranch()->GetName();
		int i = 0;
		while ((i<name.length()))	{
			if(name[i] == ':'){c++;}
			i++;
...
void Tree::Detach(Link* down, Link* up, Link*& fromdown, Link*& fromup)	{
	...
	if (degree != 2)	{
		cerr << "error in detach: node not of degree 3\n";
		cerr << degree << '\n';
	...
	fromup->SetNext(fromdown->Out());
	fromdown->Out()->SetNext(upout->Next());
	fromdown->Out()->SetNode(upout->Next()->GetNode());
	upout->SetNext(upout);
	upout->SetNode(0);
	up->SetNext(downout);
	downout->SetNext(up);
...
void Tree::Attach(Link* down, Link* up, Link* todown, Link* toup)	{
	...
	for (Link* link=toup->Next(); ((!found) && (link!=toup)); link=link->Next())	{
		if (link == todown->Out())	{
			linkprev->SetNext(up->Out());
			up->Out()->SetNext(link->Next());
			found = true;
		}
		linkprev = link;
	}
